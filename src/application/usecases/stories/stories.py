...
class RetrieveStoryAuthor(BaseUseCase):
    async def __call__(self, story_id, user_id):
        result = await self.uow.db.story_q_rep.get_author_story_by_id(story_id=story_id, user_id=user_id)
        return result
